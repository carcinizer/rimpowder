...
namespace disp{
  namespace internal {
    window_impl::window_impl(vec2<int> resolution, const std::string& window_title):
      resolution(resolution), window_title(window_title),
      window_surface(nullptr), window(nullptr) {
    ...
    int window_impl::initialise() {
      ...
      if(!SDL_CreateWindowAndRenderer(window_title.c_str(),
        resolution.x,
        resolution.y,
        0,
        &window,
    ...
    void window_impl::draw_buffer(int x_pos, int y_pos, int x_size, int y_size, void* buff, uint32_t stride) {
      ...
      SDL_Texture* texture = SDL_CreateTexture(
          ...
          SDL_TEXTUREACCESS_STREAMING, x_size, y_size);
      if(!texture) {
