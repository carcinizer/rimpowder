...
namespace disp { namespace delegates {
  // template<typename Target, void(*actuator)(Target&,
  // disp::event::event_ptr_t ev)> using namespace std::placeholders;
  ...
  class event_wrapper {
   ...
   public:
    ...
    // keyboard_fun_ptr get_keyboard_fun() { return nullptr; }
    // //keyboard_fun.target<void(unsigned char, int, int)>(); }
    ...
    static void keyboard_ev_wrap(unsigned char key, int x, int y) {
      ...
      // auto ptr =
      // std::make_shared<disp::event::event_t>(disp::event::event_t{disp::event::KEYBOARD,
      // disp::event::keyboard_event{key, shift, alt, ctrl}});
      // actuator(event_wrapper::current_instance->tg, ptr);
    }
  };

  template <typename Target, typename fnType, fnType actuator>
  event_wrapper<Target, fnType, actuator>*
      event_wrapper<Target, fnType, actuator>::current_instance = nullptr;

}; };  // namespace disp::delegates